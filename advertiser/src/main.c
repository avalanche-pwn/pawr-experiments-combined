...
struct k_poll_event events[] = {
    K_POLL_EVENT_STATIC_INITIALIZER(K_POLL_TYPE_SEM_AVAILABLE,
                                    K_POLL_MODE_NOTIFY_ONLY, &sem_connected, 0),
    K_POLL_EVENT_STATIC_INITIALIZER(K_POLL_TYPE_SEM_AVAILABLE,
                                    K_POLL_MODE_NOTIFY_ONLY, &sem_disconnected,
                                    0),
};

static struct bt_uuid_128 pawr_char_uuid = BT_UUID_INIT_128(
...
static struct bt_le_per_adv_subevent_data_params
    subevent_data_params[NUM_SUBEVENTS];
...
void remote_info_available_cb(struct bt_conn *conn,
...
int init_led() {
    if (!gpio_is_ready_dt(&master_led)) {
        return -1;
    }

    return gpio_pin_configure_dt(&master_led, GPIO_OUTPUT_ACTIVE);
}
